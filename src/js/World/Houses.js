...
export default class Houses {
  ...
  createHouse() {
    this.gltfLoader.load(HouseModel, (loaded) => {
      ...
      for (let i = 0; i < this.amount; i++) {
        ...
        newHouse.originalScale = new Vector3(1 + Math.random(), 1 + Math.random(), 1 + Math.random())
        newHouse.targetScale = newHouse.originalScale
        newHouse.targetScale.setLength(this.baseScale + Math.random() * 0.1);

        if (i > 2)
          newPos.set(oldPos.x + (Math.random() - 0.5) * this.spread.x,
            oldPos.y + (Math.sqrt(Math.random()) - 0.1) * this.spread.y,
            oldPos.z + (Math.random() - 0.5) * this.spread.z)
        else
          newPos.set((Math.random() - 0.5) * this.spread.x, 0, (Math.random() - 0.5) * this.spread.z)
    ...
    this.time.on('tick', () => {
      ...
      if (this.animationElapsed < this.animationDuration + this.amount) {
        ...
        for (let i = 0; i < this.amount; i++) {
          ...
          this.easeInOutSineVector(elapsed, h.rotation, h.startRotation, diff, this.animationDuration)
          this.easeInOutSineVector(elapsed, h.scale, new Vector3(0, 0, 0), h.targetScale, this.animationDuration)
  ...
  easeInOutSineVector(elapsed, current, start, target, duration) {
    current.set(easeInOutSine(elapsed, start.x, target.x, duration),
